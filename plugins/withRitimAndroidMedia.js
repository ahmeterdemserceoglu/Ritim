...
module.exports=function withRitimAndroidMedia(config){
 ...
 config=withDangerousMod(config,['android',async c=>{
  ...
  const javaDir=path.join(project,'app','src','main','java','com','ritim','music');
  const xmlDir=path.join(project,'app','src','main','res','xml');const layoutDir=path.join(project,'app','src','main','res','layout');
  fs.mkdirSync(javaDir,{recursive:true});fs.mkdirSync(xmlDir,{recursive:true});fs.mkdirSync(layoutDir,{recursive:true});
