...
export async function searchAllMusic(query:string):Promise<Track[]>{const q=query.trim();if(q.length<2)return[];await rememberSearch(q).catch(()=>{});const prefs=await loadPreferences();const key=`search:${normalize(q)}:${prefs.preferredQuality}:${prefs.sourcePriority.join(',')}`;const cached=await cacheGet<Track[]>(key);if(cached?.length)return cached;let raw:Track[]=[];if(await serverEnabled()){try{raw=await searchViaServer(q)}catch{}}if(!raw.length)raw=await searchDirect(q);const ranked=bestSources(raw,prefs);const filtered:Track[]=[];for(const t of ranked)if(!(await isBlocked(t)))filtered.push(t);await cacheSet(key,filtered,15*60*1000).catch(()=>{});if(prefs.debugLogging)console.log('[Ritim/search]',{query:q,raw:raw.length,results:filtered.length,quality:prefs.preferredQuality});return filtered;}
